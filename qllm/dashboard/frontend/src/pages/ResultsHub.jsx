...
export default function ResultsHub() {
  ...
  useEffect(() => {
    Promise.all([api.explore(), api.studies(), api.suites()])
      .then(([explorePayload, studiesPayload, suitesPayload]) => {
        setExplore(explorePayload)
        setStudies(studiesPayload)
  ...
  if (!explore && !error) return <div className="loading">Loading results hub...</div>
  ...
  return (
    <div>
      ...
      <div className="stat-row">
        <div className="metric-card"><div className="metric-label">Domains</div><div className="metric-value">{explore?.domains?.length || 0}</div><div className="muted">research areas inferred from runs/jobs</div></div>
        <div className="metric-card"><div className="metric-label">Tasks</div><div className="metric-value">{explore?.tasks?.length || 0}</div><div className="muted">task-level evidence slices</div></div>
        <div className="metric-card"><div className="metric-label">Datasets</div><div className="metric-value">{explore?.datasets?.length || 0}</div><div className="muted">dataset-level result views</div></div>
        <div className="metric-card"><div className="metric-label">Studies</div><div className="metric-value">{studies.length}</div><div className="muted">multi-seed protocols with reports</div></div>
      </div>
      ...
      <div className="workspace-grid">
        <section className="panel">
          ...
          <div className="research-card-list">
            {topStudies.map((study) => (
              <Link className="research-card" key={study.id} to={`/studies/${study.id}/report`}>
                <b>#{study.id} {study.name}</b>
                <span>{study.research_question || 'Multi-run protocol'}</span>
                <span>{study.job_count} jobs - {study.evidence?.label || 'pending'}</span>
              </Link>
            ))}
            {topStudies.length === 0 && <p className="muted">No studies yet. Create one from the Studies page.</p>}
          </div>
        </section>
        ...
        <section className="panel">
          ...
          <div className="research-card-list">
            ...
            {topTasks.length === 0 && <p className="muted">No task slices yet.</p>}
          </div>
        </section>
      </div>
      ...
      <div className="workspace-grid">
        <section className="panel">
          ...
          <div className="research-card-list">
            ...
            {topDatasets.length === 0 && <p className="muted">No dataset results yet.</p>}
          </div>
        </section>
