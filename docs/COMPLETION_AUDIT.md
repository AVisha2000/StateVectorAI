...
| 3 | Two-stream causality and metric labeling | Completed; causal rerun remains research-dependent | Causal prefix summaries and leakage tests are complete in `qllm/models/two_stream.py` and `tests/test_two_stream.py`; dashboard/query contracts and the approved historical correction in `RESULTS.md` section 20 label `two-stream-v1` as `teacher_forced_side_information` and `rerun_required` with no strict autoregressive conclusion. No post-repair `two-stream-causal-v2` study has completed. |
...
it is not a second independent backlog. The causal-v2 rerun remains a research
program rather than an engineering-completion requirement.
